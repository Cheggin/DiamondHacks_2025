...
//     // TODO: Optionally navigate to a detail screen
//   };
...
export default function PillHistoryTable() {
  ...
  const comparePills = async (pill1: any, pill2: any) => {
    try {
      const response = await fetch("http://100.80.14.54:5001/ddi?drug1_name="+pill1+"&drug2_name="+pill2, {
        method: "GET",
      });

      console.log("Comparing pills:", pill1, pill2);
  ...
  const handlePillPress = async (pill: any) => {
    let newSelection;
    ...
    if (newSelection.length === 2) {
      console.log(newSelection[0]['name'].split(' ')[0])
      console.log(newSelection[1]['name'].split(' ')[0])
      await comparePills(newSelection[0]['name'].split(' ')[0], newSelection[1]['name'].split(' ')[0]);
      setSelectedPills([]); // Optional: clear selection after comparing
    }
  };
